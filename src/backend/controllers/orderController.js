...
// Lấy danh sách đơn hàng (Admin/Collaborator)
...
// Cập nhật trạng thái đơn hàng (Admin/Collaborator)
...
// Lấy trạng thái tiếp theo có thể chuyển (Admin)
const getNextStatuses = async (req, res) => {
  try {
    const order = await Order.findById(req.params.id);
    if (!order) {
      return res.status(404).json({ message: 'Không tìm thấy đơn hàng' });
...
// Cập nhật trạng thái thanh toán (Admin/Collaborator)
...
// ZaloPay webhook
const zalopayWebhook = async (req, res) => {
  try {
...
// Mô phỏng thanh toán ZaloPay (Testing)
const simulateZalopay = async (req, res) => {
  try {
